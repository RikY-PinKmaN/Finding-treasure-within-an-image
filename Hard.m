...
%% Reading image
im = imread('Treasure_hardd.jpg'); % process images
imshow(im);
...
while ismember(cur_object, arrow_ind)
    [cur_object, I] = next_object_finder(cur_object, im);
...
treasure_id = [I, path(end)];
rectangle('Position', props(treasure_id(1)).BoundingBox, 'EdgeColor', 'g');
rectangle('Position', props(treasure_id(2)).BoundingBox, 'EdgeColor', 'g');
